...
q1 = (
    "SELECT AssuntoAtendimento, COUNT(*) as cnt "
    "FROM CNSAtendimento GROUP BY AssuntoAtendimento ORDER BY cnt DESC"
)
q2 = (
    "SELECT TOP 200 NumAtendimento, AssuntoAtendimento, RegInclusao "
    "FROM CNSAtendimento WHERE AssuntoAtendimento LIKE '%Implant%' "
    "ORDER BY RegInclusao DESC"
)
q3 = "SELECT COUNT(*) FROM CNSAtendimento WHERE AssuntoAtendimento = 'Implantação'"
q4 = (
    "SELECT COUNT(*) FROM CNSAtendimento "
    "WHERE LTRIM(RTRIM(AssuntoAtendimento)) = 'Implantação'"
)
q5 = (
    "SELECT TOP 50 NumAtendimento, AssuntoAtendimento, CONVERT(varbinary(max), AssuntoAtendimento) as bytes "
    "FROM CNSAtendimento WHERE AssuntoAtendimento LIKE '%Implant%'"
)
...
try:
    ...
    try:
        ...
        try:
            ...
            pass
        drv = get_db_connection.__globals__["ODBC_DRIVER"]
        srv = get_db_connection.__globals__["DB_SERVER"]
        dbn = get_db_connection.__globals__["DB_NAME"]
        conn_str = f"DRIVER={{{drv}}};SERVER={srv};DATABASE={dbn};Trusted_Connection=yes;"
